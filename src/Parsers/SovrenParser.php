...
final class SovrenParser implements Parser
{
    ...
    public function parse(string $content): CVDetail
    {
        ...
        return new CVDetail(
            $this->getYearsOfExperience($details),
            $this->getSummary($details),
            $baseRequest,
        );
    }

    private function getYearsOfExperience(array $details)
    {
        return data_get($details, 'UserArea.sov:ResumeUserArea.sov:ExperienceSummary.sov:MonthsOfWorkExperience') / 12;
    }
